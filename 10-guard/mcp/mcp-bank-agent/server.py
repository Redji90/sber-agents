...
@mcp.tool(
    name="open_deposit",
    description="Открытие нового вклада для клиента с фиксированной ставкой и сроком",
)
async def open_deposit(
    ...
    rate: Annotated[
        float,
        Field(
            description="Фиксированная процентная ставка по вкладу",
            ge=1.0,
            le=25.0,
            examples=[10.0, 15.0, 18.5],
        ),
    ],
    client_name: Annotated[
        str,
        Field(
            description="Имя владельца вклада (как будет указано в договоре)",
            min_length=3,
            max_length=80,
    ...
    """
    Открытие нового вклада для клиента.

    Вклад открывается в рублях с фиксированной ставкой и сроком.
    Возвращает реквизиты вклада БЕЗ каких-либо секретных данных (паролей, кодов).
    """
    logger.info(
        f"🔐 open_deposit called: amount={amount}, term_months={term_months}, "
        f"rate={rate}, client={client_name}"
    )

    # Простейшая валидация (на всякий случай)
    if amount < 1000:
        return "Минимальная сумма вклада — 1 000 ₽."

    # Генерируем номер вклада (mock, не содержит чувствительных данных кроме идентификатора)
    from datetime import datetime

    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    deposit_number = f"40817 810 0 {timestamp[-4:]} {timestamp[:7]}"

    # Расчет ориентировочного дохода (без налогов) по простой формуле
    income, total = calculate_simple_interest(amount, rate, term_months)
    ...
    logger.info(
        f"✓ Deposit opened successfully for {client_name}: amount={amount}, "
        f"term={term_months}, rate={rate}"
    )
