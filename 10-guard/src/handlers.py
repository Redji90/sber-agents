...
async def handle_message(message: Message):
    ...
    try:
        ...
        if agent.bank_agent is None:
            logger.error(f"Agent not initialized for chat {message.chat.id}")
            await message.answer(
                "⚠️ Агент не инициализирован. "
                "Пожалуйста, подождите или перезапустите бота."
            )
            return
        ...
        # Маскируем чувствительные данные перед отправкой в агента
        from middleware import pii_masker
        masked_text = pii_masker.mask_text(message.text)
        if masked_text != message.text:
            logger.info(f"🔒 PII masked for chat {message.chat.id}: {message.text[:50]}... -> {masked_text[:50]}...")
        
        # Создаем сообщение в формате LangChain с замаскированным текстом
        user_message = HumanMessage(content=masked_text)
        ...
        if result.get("interrupt"):
            ...
            try:
                ...
                if not action_requests:
                    logger.error(f"No action_requests in interrupt for chat {message.chat.id}")
                    await message.answer(
                        "⚠️ Ошибка при обработке запроса подтверждения. "
                        "Попробуйте еще раз."
                    )
                    return
                ...
                logger.error(f"Error processing interrupt for chat {message.chat.id}: {e}", exc_info=True)
                await message.answer(
                    "⚠️ Ошибка при обработке запроса подтверждения. "
                    "Попробуйте еще раз."
                )
        ...
        if not final_response:
            logger.warning(f"Empty answer from agent for chat {message.chat.id}")
            final_response = "Извините, не смог сформировать ответ. Попробуйте переформулировать вопрос."
        ...
        if config.SHOW_SOURCES and result.get("documents"):
            sources = format_sources(result["documents"])
            if sources:
                final_response = f"{final_response}\n\n{sources}"

        # 🔒 Маскируем чувствительные данные и в ответе бота перед отправкой пользователю
        try:
            from middleware import pii_masker
            masked_response = pii_masker.mask_text(final_response)
            if masked_response != final_response:
                logger.info(
                    f"🔒 PII masked in bot response for chat {message.chat.id}: "
                    f"{final_response[:80]}... -> {masked_response[:80]}..."
                )
            final_response_to_send = masked_response
        except Exception as e:
            # В случае любых проблем с маскированием — логируем и отправляем оригинальный текст,
            # чтобы не ломать функциональность бота.
            logger.error(f"Error masking PII in bot response for chat {message.chat.id}: {e}", exc_info=True)
            final_response_to_send = final_response
        
        await message.answer(final_response_to_send)
        ...
        try:
            ...
            logger.error(f"Failed to send error message to chat {message.chat.id}")
    except Exception as e:
        logger.error(f"Error in handle_message for chat {message.chat.id}: {e}", exc_info=True)
        
        # Определяем тип ошибки для более информативного сообщения
        error_message = "Произошла ошибка при обработке вашего сообщения."
        if "403" in str(e) or "Forbidden" in str(e) or "PermissionDenied" in str(e):
            error_message = (
                "⚠️ Ошибка доступа к API модели (403 Forbidden).\n\n"
                "Возможные причины:\n"
                "• Неверный API ключ\n"
        ...
        elif "timeout" in str(e).lower() or "таймаут" in str(e).lower():
            error_message = (
        ...
        try:
            ...
            logger.error(f"Failed to send error message to chat {message.chat.id}: {send_error}")
...
async def handle_hitl_callback(callback: CallbackQuery):
    ...
    try:
        ...
        # 🔒 Маскируем чувствительные данные в финальном ответе после HITL
        try:
            from middleware import pii_masker
            masked_response = pii_masker.mask_text(final_response)
            if masked_response != final_response:
                logger.info(
                    f"🔒 PII masked in HITL response for chat {chat_id}: "
                    f"{final_response[:80]}... -> {masked_response[:80]}..."
                )
            final_response_to_send = masked_response
        except Exception as e:
            logger.error(f"Error masking PII in HITL response for chat {chat_id}: {e}", exc_info=True)
            final_response_to_send = final_response
        
        await callback.message.answer(final_response_to_send)
