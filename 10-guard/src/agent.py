...
from middleware import ModelCallLimitMiddleware, ToolCallLimitMiddleware
from tools import rag_search
from deposit_tools import open_deposit
...
async def create_bank_agent():
    ...
    # Базовые инструменты - поиск в PDF документах и открытие вклада (локально)
    tools = [rag_search, open_deposit]
    ...
    if config.MCP_ENABLED:
        try:
            ...
            if mcp_tools:
                # Фильтруем инструменты MCP: оставляем только open_credit_card
                # Итого будет 3 инструмента: rag_search, open_deposit (локальный), open_credit_card (MCP)
                # search_products временно исключен из-за проблем с Groq API и сложной схемой
                allowed_tool_names = ["open_credit_card"]
                filtered_tools = [tool for tool in mcp_tools if tool.name in allowed_tool_names]
                
                if filtered_tools:
                    tools.extend(filtered_tools)
                    logger.info(f"✓ Connected to MCP server, loaded {len(filtered_tools)} tool(s) (filtered from {len(mcp_tools)}):")
                    for tool in filtered_tools:
                        logger.info(f"  - {tool.name}: {tool.description}")
                    
                    # Проверяем наличие open_credit_card
                    tool_names = [tool.name for tool in tools]
                    if "open_credit_card" in tool_names:
                        logger.info(f"✓ open_credit_card found in tools list (total: {len(tools)} tools)")
                    else:
                        logger.warning(f"⚠️  open_credit_card NOT found in tools list. Available: {tool_names}")
                else:
                    logger.warning(f"⚠️  No allowed tools found. Available MCP tools: {[t.name for t in mcp_tools]}")
            else:
    ...
    agent_graph = create_agent(
        ...
        middleware=[
            ...
            # Примечание: PII маскирование выполняется в handlers.py перед созданием сообщения
            HumanInTheLoopMiddleware(
                interrupt_on={
                    # Открытие кредитной карты — критичная операция
                    "open_credit_card": {
                        "allowed_decisions": ["approve", "reject"]
                    },
                    # Открытие вклада — тоже критичная операция, требующая подтверждения
                    "open_deposit": {
                        "allowed_decisions": ["approve", "reject"]
                    },
                }
    ...
    # Логируем все инструменты для отладки
    tool_names = [tool.name if hasattr(tool, 'name') else str(tool) for tool in tools]
    logger.info(f"✓ Bank agent created successfully with {len(tools)} tools and HITL middleware (PII masking in handlers)")
...
# Глобальные экземпляры middleware для лимитов
model_call_limit_middleware = ModelCallLimitMiddleware(
    limit=config.MODEL_CALL_LIMIT,
    window_seconds=config.MODEL_CALL_WINDOW
)
tool_call_limit_middleware = ToolCallLimitMiddleware(
    limit=config.TOOL_CALL_LIMIT,
    window_seconds=config.TOOL_CALL_WINDOW
)
...
async def _run_agent_stream(inputs, agent_config, chat_id: int):
    ...
    # 🔒 Проверка лимита на вызовы модели перед началом работы агента
    is_allowed, error_msg = model_call_limit_middleware.check_and_record(chat_id)
    if not is_allowed:
        logger.warning(f"🚫 Model call limit exceeded for chat {chat_id}")
        return {
            "answer": error_msg,
            "documents": [],
            "interrupt": None
    ...
    async for step in bank_agent.astream(inputs, config=agent_config):
        ...
        for node_name, update in step.items():
            if node_name != "__interrupt__" and isinstance(update, dict) and "messages" in update:
                ...
                # 🔒 Проверка лимита на вызовы инструментов
                # Если последнее сообщение - это вызов инструмента (ToolMessage)
                if isinstance(last_message, ToolMessage):
                    tool_name = getattr(last_message, 'name', 'unknown')
                    is_allowed, error_msg = tool_call_limit_middleware.check_and_record(chat_id, tool_name)
                    if not is_allowed:
                        logger.warning(f"🚫 Tool call limit exceeded for chat {chat_id} (tool: {tool_name})")
                        return {
                            "answer": error_msg,
                            "documents": [],
                            "interrupt": None
                    ...
                    # AIMessage с tool_calls означает, что агент хочет вызвать инструмент
                    # Проверяем лимит перед фактическим вызовом
