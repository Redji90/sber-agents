...
from src.app.rag.chain import build_rag_chain, create_retriever_for_mode
from src.app.synthesis.dataset_synthesizer import synthesize_dataset
from src.app.bot.utils import send_message_with_retry, safe_answer
...
async def command_help_handler(message: types.Message) -> None:
    ...
    await send_message_with_retry(lambda: message.answer(help_text))
...
async def command_reset_handler(message: types.Message) -> None:
    ...
    session_manager.clear_session(user_id)
    await send_message_with_retry(
        lambda: message.answer("История диалога очищена. Начинаем с чистого листа.")
    )
...
async def command_index_handler(message: types.Message) -> None:
    ...
    if status == "running":
        ...
        await send_message_with_retry(
            lambda: message.answer("Индексация уже выполняется. Проверьте статус позже командой /index_status.")
        )
        return

    logger.info("Запуск переиндексации по запросу пользователя %s.", user_id)
    await send_message_with_retry(
        lambda: message.answer("Запускаю переиндексацию данных. Проверьте статус через /index_status.")
    )
...
async def command_index_status_handler(message: types.Message) -> None:
    ...
    await send_message_with_retry(lambda: message.answer(status_message))
...
async def handle_text_message(message: types.Message) -> None:
    ...
    try:
        ...
        # Игнорируем ошибки отправки typing action, это не критично
        logger.debug("Не удалось отправить typing action: %s", e)

    chat_history: List[BaseMessage] = session_manager.get_messages(user_id)
    
    # Создаём retriever в зависимости от режима RAG_MODE
    try:
        semantic_retriever = manager.get_retriever()
        # Для hybrid режимов нужны документы для BM25 индексации
        documents_for_bm25 = None
        if config.RAG_MODE in ("hybrid", "hybrid+reranker"):
            documents_for_bm25 = manager.get_documents()
            if not documents_for_bm25:
                logger.warning(
                    "Режим %s требует документы для BM25, но они отсутствуют. "
                    "Запустите /index для индексации документов.",
                    config.RAG_MODE
                )
                await safe_answer(
                    message,
                    f"Режим {config.RAG_MODE} требует индексированные документы. "
                    "Запустите /index и дождитесь завершения, затем повторите вопрос."
                )
                return
        
        retriever = create_retriever_for_mode(semantic_retriever, documents_for_bm25)
        rag_chain = build_rag_chain(retriever)
    except ValueError as exc:
        # Ошибка создания retriever (например, отсутствуют документы для hybrid режима)
        logger.error("Ошибка создания retriever для режима %s: %s", config.RAG_MODE, exc)
        await safe_answer(
            message,
            f"Не удалось создать retriever для режима {config.RAG_MODE}. "
            f"Ошибка: {str(exc)}. Запустите /index для индексации документов."
        )
        return

    logger.info(
        "Обработка запроса от пользователя %s. Режим RAG: %s, длина запроса: %s, история: %s реплик",
        user_id,
        config.RAG_MODE,
        len(message.text),
        len(chat_history),
    ...
    try:
        ...
        logger.info(
            "Ответ пользователю %s подготовлен. Режим RAG: %s, документов в ответе: %s, длина ответа: %s",
            user_id,
            config.RAG_MODE,
            len(documents),
            len(answer),
        ...
        # Обработка rate limit ошибок от Groq API
        logger.warning(
            "Rate limit при запросе к LLM для пользователя %s: %s. "
            "Клиент автоматически повторит запрос с экспоненциальной задержкой.",
            user_id,
            exc
        )
        # ChatOpenAI уже настроен с max_retries=3, поэтому он автоматически повторит запрос
        # Но если все попытки исчерпаны, сообщаем пользователю
        await safe_answer(
            message,
            "Сервис временно перегружен. Пожалуйста, подождите немного и попробуйте снова. "
            "Запрос автоматически повторяется..."
        )
        ...
        # Проверяем, является ли это rate limit ошибкой по строке (на случай других типов ошибок)
        ...
        if is_rate_limit:
            logger.warning(
                "Rate limit при запросе к LLM для пользователя %s: %s", user_id, exc
            )
            await safe_answer(
                message,
                "Сервис временно перегружен. Пожалуйста, подождите немного и попробуйте снова."
